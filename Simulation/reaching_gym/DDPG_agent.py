# -*- coding: utf-8 -*-
"""
Created on Mon Jul  5 00:41:26 2021
...
# TODO: the follow definations are not decent, to be modified
obs_dim = (10,)
obs_num = 10
act_num = 4
act_dim = (4,)
...
# 	    self.env  = env
        ...
        self.actor_critic_grad = tf.placeholder(tf.float32,
                                                [None, act_num]) # where we will feed de/dC (from critic)
    ...
    def create_actor_model(self):
        ...
        output = Dense(act_num, activation='tanh')(h3)
    ...
    def act(self, cur_state):
        ...
        if np.random.random() < self.epsilon:
            return np.array([[2 * np.random.normal(),2 * np.random.normal(),2 * np.random.normal(),2 * np.random.normal()]])
        else:
