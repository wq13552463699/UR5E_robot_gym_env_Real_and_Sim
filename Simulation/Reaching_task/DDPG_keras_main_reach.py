...
for episode in range(EPISODES):
    ...
    if episode % 500 == 0:
        actor_critic.actor_model.save_weights(str(episode)+"_"+"actor"+".h5")
        actor_critic.target_actor_model.save_weights(str(episode)+"_"+"actor_target"+".h5")
        actor_critic.critic_model.save_weights(str(episode)+"_"+"critic"+".h5")
        actor_critic.target_critic_model.save_weights(str(episode)+"_"+"critic_target"+".h5")
        
    if episode % 100 == 0:
        history["Episode"].append(episode)
        history["Success_rate"].append(global_success_time / episode)
        df = pd.DataFrame(history)
        df.to_csv("history.csv",index=False,sep=',')